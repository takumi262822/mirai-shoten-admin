...
- 本番APIはCORSでフロントエンド本番ドメイン（https://mirai-shoten-admin.vercel.app）のみ許可し、セキュリティを強化しています。
- APIサーバーは直接ブラウザでアクセスすると説明文が表示されます。
- フロントエンドからの利用時のみAPI通信が許可されます。
- JWT認証・バリデーション・XSS対策を実装済み。パスワードはハッシュ化して保存。
- CI/CD（GitHub Actions）・自動テスト（Jest+Supertest）導入済み。
- 今後は多要素認証やIP制限など、さらなるセキュリティ強化も検討。
